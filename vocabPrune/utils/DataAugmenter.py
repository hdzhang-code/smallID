...
#
    ...
    def dataAugCloseGPT3(self, uttList, labList, augNum = 5, device=None):
        ...
        if os.path.exists(cacheFilePath):   # hit cache
            ...
            model = GPTJForCausalLM.from_pretrained("EleutherAI/gpt-j-6B", revision="float16", torch_dtype=torch.float16, low_cpu_mem_usage=True, cache_dir='.cache')
    ...
    def extractSubVocab(self, uttListOri, labListOri, augNum = 5, device=None, returnTensorDataset = True, mergeOriData = True, oriTensorDataset=None, dataset = None, tokenizerOri=None, vocabKeepSize = 1):
        # check cache
        cacheFilePath = os.path.join(self.cachePath, self.cacheFile) 
        if os.path.exists(cacheFilePath):   # hit cache
            logger.info(f"Reading from cache: {cacheFilePath}")
        ...
        if returnTensorDataset and mergeOriData:
            ...
            for u in uttListAll:
                encode = tokenizerOri.encode(u)
                tokens = tokenizerOri.convert_ids_to_tokens(encode)
                tokenAllList.extend(tokens)
                ut = tokenizerOri(u)

            # clean up Unicode
            tokenAllListNew = []
            for token in tokenAllList:
                if any(ord(x) > 127 for x in token):
                    logger.info(f"Remove due to unicode: {token}")
                    # string is *not* ASCII
                    pass
                else:
                    tokenAllListNew.append(token)
            tokenAllList = tokenAllListNew

            # top-K high freqency
            tokenToFrequency = {}
            for token in tokenAllList:
                if token not in tokenToFrequency:
                    tokenToFrequency[token] = 0
                tokenToFrequency[token] = tokenToFrequency[token] + 1
            sorted_tokenToFrequency = sorted(tokenToFrequency.items(), key=lambda x:x[1], reverse=True)
            selectedNum = vocabKeepSize
            sorted_tokenToFrequency = sorted_tokenToFrequency[0:selectedNum]
            vocab = set([token for (token, frequency) in sorted_tokenToFrequency])
            return vocab
        else:
            return newUttList, newLabList
