...
class StatisticsCollector(StatisticsCollectorBase):
    ...
    def collectIsotropyMeature(self, model, tokenizer, dataset, centralize=False):
        ...
        with torch.no_grad():
            # 1. get sentence of word embeddings in the test dataset
            labTensorData = makeTrainExamples(dataset.getTokList(), tokenizer, dataset.getLabID(), mode='multi-class')
            batchSize = 64
    ...
    ##
    # @brief  dump feature vectors into disk.
    #
    # @param model
    # @param path
    #
    # @return 
    def dumpFeatures(self, model, tokenizer, dataset, path):
        model.eval()
