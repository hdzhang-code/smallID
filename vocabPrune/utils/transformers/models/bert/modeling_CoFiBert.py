...
from ...modeling_utils import (apply_chunking_to_forward,
...
from ...file_utils import hf_bucket_url, cached_path
...
class CoFiBertForSequenceClassification(BertForSequenceClassification):
    def __init__(self, config):
        ...
        self.do_layer_distill = True

        if self.do_layer_distill:
            self.layer_transformation = nn.Linear(
                config.hidden_size, config.hidden_size)
            # init layer transformation as the eye matrix
            self.layer_transformation.weight.data.copy_(
                torch.eye(len(self.layer_transformation.weight)))
            self.layer_transformation.bias.data.fill_(0)
        else:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        load_pruned_model(model, weights, silence=silence)
        return model
...
class CoFiBertLayer(BertLayer):
    ...
    def forward(
        ...
        if self.intermediate.dense is None:   # if intermedaite is pruned, then just return the attention output
            layer_output = attention_output
        else:                                 # if intermedaite is not pruned, then forward the FFN
...
class CoFiBertAttention(BertAttention):
    ...
    def prune_heads(self, heads):
        ...
        # get the index of the remaining index
...
class CoFiBertSelfOutput(BertSelfOutput):
    ...
    def forward(self, hidden_states, input_tensor, head_layer_z=None, hidden_z=None, inference=False):
        if hidden_states is None:   # self attention is totally pruned
            return input_tensor
        hidden_states = self.dense(hidden_states)
        if head_layer_z is not None:
            hidden_states = hidden_states.mul(head_layer_z)
        if not inference and hidden_states.sum().eq(0).item():   # if it is training, and the hidden states is 0.0, either because of 0 mask of head layer or 0 mask of heads, then output hidden_states+input_tesnor, which is actually the input_tensor, the input of self attention module. Dropout and layer norm are ignored.
            hidden_states = hidden_states + input_tensor
        else:      # if it is inference, or, if hidden states is valid (there is some information), then follow the normal flow, i.e. dropout and layer norm.
            if hidden_z is not None:
...
class CoFiBertOutput(BertOutput):
    ...
    def forward(self, hidden_states, input_tensor, mlp_z, hidden_z=None, inference=False):
        ...
        if mlp_z is not None:
            hidden_states *= mlp_z
        if not inference and hidden_states.sum().eq(0).item():   # if it is training, and hidden_states is not valid (all 0, which is due to mlp_z=0 or the former mask such as the mask of intermediate layer), then ignore layer norm and dropout, simply return hidden_states+input_tensor, which is indeed the input tensor of the entire FFN.
