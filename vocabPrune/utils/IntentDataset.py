...
class IntentDataset():
    ...
    def tokenize(self, tokenizer, vocabList = None):
        if self.trainPart is not None:
            self.trainPart.tokenize(tokenizer, vocabList = vocabList) 
        if self.valPart is not None:
            self.valPart.tokenize(tokenizer, vocabList = vocabList)
        if self.testPart is not None:
            self.testPart.tokenize(tokenizer, vocabList = vocabList)
