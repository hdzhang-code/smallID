...
##
# @brief Different from FewShotEvaluator, SupervisedEvaluator evaluate model not by sampling tasks, but by directly fitting a classifier and evaluate it on the entire dataset. It is still a few-shot scenario, but we do not sample N-way. We directly sample K-shot for each class to fit a classifier in the training partition and then evaluate on the test partition.
...
##
# @brief InTaskCoFiModelEvaluator used to do evaluation. FineTuneEvaluator is for meta-evaluation, there is a concept of task. InTaskFineTuneEvaluator imitates supervised learning, it is supposed to be used on a standard supervised scenario, we have training data and testing data. It is used to evaluate CoFi model, the model can be either already pruned or not pruned. The K-shot data is used to equip a linear classifier, and then test on test data.
class InTaskCoFiModelEvaluator(EvaluatorBase):
    def __init__(self, evalParam, dataset: IntentDataset):
        super(InTaskCoFiModelEvaluator, self).__init__()
        self.batch_size = evalParam['batch_size']
        self.dataset      = dataset

    ##
    # @brief use KShotDataLoader to build up a model for classification, report the performance. KShotDataLoader is used to get the embeddings and then compose a linear classsifier.
    #
    # @param model
    # @param tokenizer
    ...
    #
    # @return 
    def evaluate(self, model, tokenizer, KShotDataLoader, logLevel='DEBUG'):
        # set_seed(self.seed)   # reset seed so that the seed can control the following random data sample process.
        # evaluate on test partition
        model.eval()
        with torch.no_grad():
