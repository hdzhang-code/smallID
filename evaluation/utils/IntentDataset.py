...
##
# @brief IntentDataset contains three partitions: train, dev and test. Some partions my be null. Each partition contains data, including utterances, label, dom, tokens, etc. All partitions in one IntentDataset share the same label idx map: label name -> label idx.
class IntentDataset():
    ...
    def buildVocabMap(self,tok, vocabList, model):
        '''
        # clean up vocab list
        vocabListNew = []
        for token in vocabList:
            if any(ord(x) > 127 for x in token):
                ...
                vocabListNew.append(token)
        vocabList = vocabListNew
        '''
        ...
        for token in vocabList:
            ...
            tokenIDList.append(token_id)
        embeddingAll = model.backbone.bert.embeddings.word_embeddings.weight
        distMat = torch.cdist(embeddingAll, embeddingAll)

        # choose the nearest one
        tokenCount = embeddingAll.shape[0]
        vocabMap = {}
        distVocab2All = distMat[tokenIDList]
        minDist, indices = torch.min(distVocab2All, 0)
        for tokenID in range(tokenCount):
            replaceTokenID = tokenIDList[indices[tokenID]]
            oriToken     = tok.convert_ids_to_tokens(tokenID)
            replaceToken = tok.convert_ids_to_tokens(replaceTokenID)
            logger.info(f"Replace token: {oriToken} -> {replaceToken}")
            vocabMap[oriToken] = replaceToken
    ...
    def tokenize(self, tokenizer, vocabList = None, vocabMap = None):

        if self.trainPart is not None:
            self.trainPart.tokenize(tokenizer, vocabList = vocabList, vocabMap = vocabMap) 
        if self.valPart is not None:
            self.valPart.tokenize(tokenizer, vocabList = vocabList, vocabMap = vocabMap)
        if self.testPart is not None:
            self.testPart.tokenize(tokenizer, vocabList = vocabList, vocabMap = vocabMap)
...
class Partition():
    ...
    def randomSliceTorchDataset(self, k, tokenizer, returnUttListLabList=True):
        ...
        tensorDataset = TensorDataset(label,
                                 ...
                                 paddedTokens['token_type_ids'],
                                 paddedTokens['attention_mask'])
        if returnUttListLabList:
            return tensorDataset, uttList, labList
        else:
            return tensorDataset

    ##
    # @brief randomly sample m data for each label. This function is use for knowledge distillation, but label is also returned for checking.
    #
    # @param m, m data per label
    # @param tokenizer, 
    # @param exceptUttList, these utterances will be filtered out during ramdom selection.
    # @param mergeExcept, True: merge the selected samples and the selected samples. False: return only selected samples. 
    #
    # @return 
    def randomSliceUnlabeledTorchDataset(self, m, tokenizer, exceptUttList = None, mergeExcept=False):
        assert m>=0, 'randomSliceUnlabeledTorchDataset(), m>=0'

        k = m
        # get data inds to be filtered
        exceptDataInds = []
        if exceptUttList is not None:
            for dataInd, utt in enumerate(self.uttList):
                if utt in exceptUttList:
                    exceptDataInds.append(dataInd)
        ...
        for labID in labID2DataInds:
            dataInds = cp.deepcopy(labID2DataInds[labID])
            dataInds = [dataInd for dataInd in dataInds if dataInd not in exceptDataInds]
            random.shuffle(dataInds)
            selectDataInds.extend(dataInds[:k])

        if mergeExcept:
            selectDataInds = selectDataInds + exceptDataInds
    ...
    def tokenize(self, tokenizer, vocabList = None, vocabMap = None):
        ...
        for u in self.uttList:
            ...
            input_ids      = ut['input_ids']
            token_type_ids = ut['token_type_ids']
            attention_mask = ut['attention_mask']
            tokens = tokenizer.convert_ids_to_tokens(input_ids)
            input_ids_new = []
            token_type_ids_new = []
            attention_mask_new = []
            for token, input_id, token_type_id, mask in zip(tokens, input_ids, token_type_ids, attention_mask):
                if token in vocabList:
                    input_ids_new.append(input_id)
                    token_type_ids_new.append(token_type_id)
                    attention_mask_new.append(mask)
                else:
                    tokenReplace = vocabMap[token]
                    logger.info(f'\'{token}\' not in vocabulary. Replace it with \'{tokenReplace}\'')
                    input_ids_new.append(tokenizer.convert_tokens_to_ids(tokenReplace))
                    token_type_ids_new.append(token_type_id)
                    attention_mask_new.append(mask)
            ut['input_ids'] = input_ids_new
            ut['token_type_ids'] = token_type_ids_new
            ut['attention_mask'] = attention_mask_new
