...
class IntentBERTCoFi(nn.Module):
    ...
    def fromTokenToEmbedding(self, token):
        embedding = self.backbone.bert.embeddings.word_embeddings.weight[token]
        return embedding

    def loadPCAVocab(self, pcaVocab):
        self.vocabPCAParam             = pcaVocab[VOCAB_PCA_PARAM]
        self.vocab_embedding_lower_dim = pcaVocab[VOCAB_EMBEDDING_LOWER_DIM]
        self.vocab_pca_token_list      = pcaVocab[VOCAB_PCA_TOKEN_LIST]

        # setup an embedding for lower dimensional one.
        maxTokenID = max(self.vocab_pca_token_list)
        lowerDim = self.vocab_embedding_lower_dim.shape[1]
        pdb.set_trace()
        self.word_embedding_lowerDim = nn.Embedding(maxTokenID + 1, lowerDim)
        with torch.no_grad():
            vocabEmbeddingTensor = torch.zeros(maxTokenID + 1, lowerDim)
            for tokenID, embeddingLower in zip(self.vocab_pca_token_list, self.vocab_embedding_lower_dim):
                vocabEmbeddingTensor[tokenID] = torch.from_numpy(embeddingLower)
            self.word_embedding_lowerDim.weight.copy_(vocabEmbeddingTensor)
        self.word_embedding_lowerDim.to(self.device)

        # setup pca recover parameters 
        self.pcaMatrixRecover = torch.from_numpy(self.vocabPCAParam[0])
        self.pcaMean = torch.from_numpy(self.vocabPCAParam[1])
        self.pcaMatrixRecover = self.pcaMatrixRecover.to(self.device)
        self.pcaMean = self.pcaMean.to(self.device)
    ...
    def getTokenEmbeddingSelfVocab(self, X):
        embedidngLower = self.word_embedding_lowerDim(X['input_ids']).to(self.device)

        embedding_hat = embedidngLower @ self.pcaMatrixRecover + self.pcaMean
        return embedding_hat

    def getUttEmbeddings(self, X, returnAllLayers=False):
        # BERT forward
        if True:
            inputs_embeds = self.getTokenEmbeddingSelfVocab(X)
            X['input_ids'] = None
            outputs = self.backbone(**X, output_hidden_states=True, inputs_embeds = inputs_embeds)
        else:
            outputs = self.backbone(**X, output_hidden_states=True)
        # outputs = self.backbone(**X, output_hidden_states=True, vocabPCAParam = self.vocabPCAParam, vocab_embedding_lower_dim = self.vocab_embedding_lower_dim)
