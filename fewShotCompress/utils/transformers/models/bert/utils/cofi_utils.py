import torch
import os
from .....transformers.models.auto.modeling_auto import AutoConfig, BertForSequenceClassification
...
def load_model_with_zs(model_path, model_class, zs=None, silence=False):
    ...
    prune_model_with_z(zs, model, silence=silence)
...
def prune_model_with_z(zs, model, silence=False):
    if zs is None:
        return None, None
    bert = model.bert if hasattr(model, "bert") else model.roberta
   
    # prune heads, including mask of each head and mask of the head layer
    if "head_z" in zs:
    ...
    # prune intermedaite layer, including mask for intermediate dimensions and mask for intermediate layer
    ...
    if "hidden_z" in zs:
        ...
        # prune word embeddings
        bert.embeddings.word_embeddings.weight = torch.nn.parameter.Parameter(bert.embeddings.word_embeddings.weight.index_select(1, index).clone().detach())
        bert.embeddings.word_embeddings.embedding_dim = index.shape[0]
        bert.embeddings.position_embeddings.weight = torch.nn.parameter.Parameter(bert.embeddings.position_embeddings.weight.index_select(1, index).clone().detach())
        bert.embeddings.position_embeddings.embedding_dim = index.shape[0]
        bert.embeddings.token_type_embeddings.weight = torch.nn.parameter.Parameter(bert.embeddings.token_type_embeddings.weight.index_select(1, index).clone().detach())
        bert.embeddings.token_type_embeddings.embedding_dim = index.shape[0]
        prune_layer_norm(bert.embeddings.LayerNorm, index)

        # prune hidden dimension, query, key, value, selfoutput, layer normalization, intermediate layer, output layer
    ...
    # prune intermediate layer and its dimensions
...
def prune_intermediate_layers(model, keep_dims):
    ...
    for layer in keep_dims:
        if len(keep_dims[layer]) == 0:  # if the whole intermediate layer is deactivated
...
def load_pruned_model(model, weights, silence=False):
    config = model.config
